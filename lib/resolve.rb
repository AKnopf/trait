module Traits
  ...
  class Resolve
    ...
    # @param [Proc] lambda
    #   @see Resolve#lambda
    def initialize(order, link_mode, link_operator, lambda = -> { raise "no lambda defined for resolve" })
    ...
    def validate_with_pattern
      ...
      raise "Order must be an Array<Trait> but is #{order.inspect}" unless order.respond_to?(:all?) and order.all? {
          |trait|
